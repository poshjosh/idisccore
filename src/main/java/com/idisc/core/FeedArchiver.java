...
public class FeedArchiver {
    private transient static final Logger LOG = Logger.getLogger(FeedArchiver.class.getName());
  ...
  public int archiveFeedsBefore(Date date, int batchSize) {
    ...
    LOG.log(Level.FINE, "Archiving feeds before: {0}", date);
    ...
    for (;;) {
      ...
      try(Select<Integer> dao = jpaContext.getDaoForSelect(Feed.class, Integer.class)){
        ...
        LOG.log(Level.FINE, "Offset: {0}, batchSize: {1}, batch: {2}", 
            new Object[]{offset, batchSize, sizeOfBatch});
          ...
          for(Integer feedid : feedids) {
            ...
            if(feed.getFeeddate().before(THRESH_HOLD)) {
              ...
              if(LOG.isLoggable(Level.FINE)) {
                LOG.log(Level.FINE, "Archive of feed not executed. Reason: Feeddate not set for feed with ID: {0}, title: {1}", 
                        new Object[]{feed.getFeedid(), feed.getTitle()});
              }
              continue;
            }
            ...
            if(!exists) {
              ...
              if(LOG.isLoggable(Level.FINER)) {
                LOG.log(Level.FINER, 
                    ...
                    new Object[]{feed.getFeedid(), feed.getFeeddate(), feed.getTitle()});
              }
            }
