 ...
 * @author poshjosh
 */
public class DownloadSyndFeedTask extends AbstractStoppableTask {
    ...
    private final Class cls = DownloadSyndFeedTask.class;
    
    private final XLogger logger = XLogger.getInstance();
    
    private final Collection<Feed> resultBuffer;
    
    public DownloadSyndFeedTask(String feedName, String url, 
      ...
      try
      {
        ...
        for (Object obj : entries)
        {
          ...
          if(contentStrBuilder.length() == 0) {
            ...
            Site site = com.idisc.core.Util.findSite(this.feedName, this.sitetype, true);
            if (site == null) {
            ...
            boolean add = (this.acceptDuplicates) || (!com.idisc.core.Util.isInDatabase(feedCtrl, reusedFeedParams, feed));
            if (add)
            {
        ...
        logger.log(Level.FINER, "Added {0} Feed records for: {1} = {2}", cls, Integer.valueOf(added), this.feedName, url);
...
//        logger.log(Level.WARNING, "", cls, e);
      ...
      catch (RuntimeException e)
      {
        ...
        logger.log(Level.FINE, "For RSS: {0} added {1} feeds", cls, url, this.resultBuffer == null ? null : this.resultBuffer.size());
      }
    }
