...
long mb4 = com.bc.util.Util.availableMemory();
        ...
        try (FeedComparatorUserHitsOnSite fc = new FeedComparatorUserHitsOnSite(
                cf.getEntityManager(Feedhit.class), installation, false)) {
            Collections.sort(feeds, fc);
        }
this.logTimeAndMemoryConsumed(feeds.size(), tb4, mb4);
this.printFeedids(feeds);
        try (FeedComparatorUserHitsOnSite fc = new FeedComparatorUserHitsOnSite(
                cf.getEntityManager(Feedhit.class))) {
            Collections.sort(feeds, fc);
        }
this.logTimeAndMemoryConsumed(feeds.size(), tb4, mb4);
this.printFeedids(feeds);
